import{K as Z,zt as tt}from"./main-ICH7CP7G.js";var g=`questify_gemini_api_key`;var y=[`gemini-3.7-flash`,`gemini-3.6-flash`];function b(t){return`https://generativelanguage.googleapis.com/v1beta/models/${t}:generateContent`}var c={type:`OBJECT`,properties:{questions:{type:`ARRAY`,items:{type:`OBJECT`,properties:{type:{type:`STRING`,enum:[`MCQ`,`CQ`]},question:{type:`STRING`},marks:{type:`NUMBER`},options:{type:`ARRAY`,items:{type:`STRING`}},correctAnswer:{type:`INTEGER`},multiSelect:{type:`BOOLEAN`},correctAnswers:{type:`ARRAY`,items:{type:`INTEGER`}},explanation:{type:`STRING`},source:{type:`STRING`},subjectHint:{type:`STRING`}},required:[`type`,`question`,`marks`]}}},required:[`questions`]};var E=`You are given an exam question paper as an image or PDF. Extract every question into structured data.
...
- Return only the explanation text itself \u2014 no preamble like "Here is the explanation:".`}var f=class t{getApiKey(){return localStorage.getItem(g)??``}setApiKey(e){localStorage.setItem(g,e.trim())}hasApiKey(){return this.getApiKey().length>0}async extractQuestions(e){let n=await k(e),i={inline_data:{mime_type:e.type||`application/pdf`,data:n}};return u(await this.fetchGeminiText([i,{text:E}],c))}async extractFromText(e){let n=q.replace(`{{TEXT}}`,e);return u(await this.fetchGeminiText([{text:n}],c))}async generateFromText(e,n,i){let o=T(n,i).replace(`{{TEXT}}`,e);return u(await this.fetchGeminiText([{text:o}],c))}async explainSolution(e){let n=A(e);return(await this.fetchGeminiText([{text:n}])).trim()}async fetchGeminiText(e,n){let i=this.getApiKey();if(!i)throw new Error(`Add your Gemini API key first.`);let o=3,s;for(let w of y)for(let a=1;a<=o;a++){let r=await fetch(b(w),{method:`POST`,headers:{"Content-Type":`application/json`,"x-goog-api-key":i},body:JSON.stringify({contents:[{parts:e}],generationConfig:Z({maxOutputTokens:65536},n?{responseMimeType:`application/json`,responseSchema:n}:{})})});if(r.ok){let p=(await r.json()).candidates?.[0],h=p?.content?.parts?.[0]?.text;if(!h){let l=p?.finishReason;throw new Error(l===`MAX_TOKENS`?`This paper is too large to extract in one go — Gemini ran out of output space. Split it into smaller batches (e.g. 30-50 questions per upload/paste) and try each separately.`:l&&l!==`STOP`?`Gemini stopped without a result (${l}) \u2014 try again or use different input.`:`Gemini returned an empty result — try again or use different input.`)}return h}let x=await r.text().catch(()=>``);if(s=new Error(`Gemini request failed (${r.status}): ${x.slice(0,300)||r.statusText}`),r.status===404)break;if(!(r.status===429||r.status===503))throw s;a<o&&await C(Math.min(1500*2**(a-1),8e3))}throw s&&/\b(429|503)\b/.test(s.message)?new Error(`${s.message}

Gemini is overloaded/rate-limited on Google's side right now \u2014 this already retried automatically across ${y.length} models. It usually clears within a minute or two; wait a bit and try again.`):s}static ɵfac=function(n){return new(n||t)};static ɵprov=tt({token:t,factory:t.ɵfac,providedIn:`root`})};function u(t){let e;try{e=JSON.parse(t)}catch{throw new Error(`Could not parse the generated questions — try again. If this is a large paper (many dozens of questions), try splitting it into smaller batches instead of one big upload.`)}return e.questions??[]}function C(t){return new Promise(e=>setTimeout(e,t))}function k(t){return new Promise((e,n)=>{let i=new FileReader;i.onload=()=>{let o=i.result;e(o.split(`,`)[1]??``)},i.onerror=()=>n(i.error),i.readAsDataURL(t)})}export{f as t};